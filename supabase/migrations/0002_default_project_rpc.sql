drop function if exists public.ensure_default_project(text, text);

create function public.ensure_default_project(project_name text, project_slug text)
...
begin
  ...
  select p.id, current_user_id, 'owner'
