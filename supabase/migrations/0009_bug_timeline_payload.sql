  ...
  from (
    ...
    left join lateral (
      select coalesce(
        jsonb_agg(
          jsonb_build_object(
            'id', public.bug_comments.id,
            'body', public.bug_comments.body,
            'createdAt', public.bug_comments.created_at,
            'updatedAt', public.bug_comments.updated_at,
            'authorEmail', public.profiles.email
          )
          order by public.bug_comments.created_at desc
        ),
        '[]'::jsonb
      ) as items
