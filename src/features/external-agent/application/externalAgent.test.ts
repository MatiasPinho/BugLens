...
describe('externalAgent', () => {
  ...
  it('ejecuta el agente externo desde el repositorio local configurado', async () => {
    ...
    try {
      const result = await runExternalAgent(script.command, makeBug(), 30_000, undefined, [
  ...
  it('migra el comando legacy de OpenCode con cat a --file', async () => {
    ...
    try {
      const result = await runExternalAgent('opencode run "$(cat {promptFile})"', makeBug(), 30_000)
  ...
  it('notifica salida parcial mientras el comando sigue corriendo', async () => {
    ...
    try {
      const result = await runExternalAgent(script.command, makeBug(), 30_000, (progress) =>
